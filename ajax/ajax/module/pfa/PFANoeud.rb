...
-strokewidth 3
-fill black
-draw "line #{left},#{top} #{left+1},#{bottom+4*PFA::LINE_HEIGHT} "
#{draw_command_other}
  CMD
end
...
def mark_horloge
  ...
  ihorloge = Horloge.new(horloge: horloge, top:bottom + 2, left:hcenter)
  ihorloge.magick_code
end

def horloge; @horloge ||= time.to_i.to_horloge end
def time ; @time ||= data['time'].to_f    end
def type ; @type ||= PFA.type_of(ntype)   end
def mark ; @mark ||= PFA.mark_of(ntype)   end
def ntype
  @ntype ||= data['type'].split(':')[1]
end
